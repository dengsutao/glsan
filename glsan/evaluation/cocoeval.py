...
class COCOeval:
    ...
    def evaluate(self):
        ...
        maxDet = p.maxDets[-1]
        self.evalImgs = [evaluateImg(imgId, catId, aptrtoRng, maxDet)
                         for catId in catIds
                         for aptrtoRng in p.aptrtoRng
                         for imgId in p.imgIds
                         ]
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        aspect_ratios = [d['bbox'][2] / d['bbox'][3] for d in dt]
        a = np.array(
            [aspect_ratio < aRng[0] or aspect_ratio > aRng[1] for aspect_ratio in aspect_ratios]
        ).reshape((1, len(dt)))
    ...
    def accumulate(self, p=None):
        ...
        setK = set(catIds)
        setA = set(map(tuple, _pe.aptrtoRng))
        setM = set(_pe.maxDets)
        setI = set(_pe.imgIds)
        # get inds to evaluate
        k_list = [n for n, k in enumerate(p.catIds) if k in setK]
        m_list = [m for n, m in enumerate(p.maxDets) if m in setM]
        a_list = [n for n, a in enumerate(map(lambda x: tuple(x), p.aptrtoRng)) if a in setA]
        i_list = [n for n, i in enumerate(p.imgIds) if i in setI]
        I0 = len(_pe.imgIds)
        A0 = len(_pe.aptrtoRng)
    ...
    def summarize(self):
        ...
        def _summarize(ap=1, iouThr=None, aptrtoRng='all', maxDets=100):
            p = self.params
            iStr = ' {:<18} {} @[ IoU={:<9} | w/h={:>6s} | maxDets={:>3d} ] = {:0.3f}'
            titleStr = 'Average Precision' if ap == 1 else 'Average Recall'
            typeStr = '(AP)' if ap == 1 else '(AR)'
            iouStr = '{:0.2f}:{:0.2f}'.format(p.iouThrs[0], p.iouThrs[-1]) \
                if iouThr is None else '{:0.2f}'.format(iouThr)

            aind = [i for i, aRng in enumerate(p.aptrtoRngLbl) if aRng == aptrtoRng]
            ...
            print(iStr.format(titleStr, typeStr, iouStr, aptrtoRng, maxDets, mean_s))
            return mean_s
        ...
        def _summarizeDets():
            ...
            stats.append(_summarize(1, aptrtoRng='1/3 - 3', maxDets=self.params.maxDets[2]))
            ...
            stats.append(_summarize(0, aptrtoRng='1/3 - 3', maxDets=self.params.maxDets[2]))
        ...
        def _summarizeKps():
            ...
            stats[3] = _summarize(1, maxDets=20, aptrtoRng='medium')
            stats[4] = _summarize(1, maxDets=20, aptrtoRng='large')
            stats[5] = _summarize(0, maxDets=20)
            stats[6] = _summarize(0, maxDets=20, iouThr=.5)
            stats[7] = _summarize(0, maxDets=20, iouThr=.75)
            stats[8] = _summarize(0, maxDets=20, aptrtoRng='medium')
            stats[9] = _summarize(0, maxDets=20, aptrtoRng='large')
            return stats
        ...
        if iouType == 'segm' or iouType == 'bbox':
            ...
            raise ValueError("Unknown iouType, got {}".format(iouType))
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.aptrtoRng = [[0 / 1, 1e5 / 1], [0 / 1, 1 / 5], [1 / 5, 1 / 3], [1 / 3, 3 / 1], [3 / 1, 5 / 1],
                          [5 / 1, 1e5 / 1]]
        self.aptrtoRngLbl = ['all', ' 0  - 1/5', '1/5 - 1/3', '1/3 - 3', '3/1 - 5/1', '5/1 - INF']
        self.useCats = 1
